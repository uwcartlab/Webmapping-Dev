...
Note that in Example 2.8, `d` still holds each of our array values, only now each value is an object with two properties (`state` and `energy`). Thus, on on line 27, we use `d.state` to assign each state name as the circle `id`, and on lines 31 and 40, we use `d.energy` to access the `energy` value of each object. Figure 2.3 shows our state circles on the page and in the HTML.

![figure8.2.3.png](img/figure8.2.3.png)

###### Figure 2.3: A bubble chart of state energy production.

> ### **Using properly formatted D3 code blocks, create a bubble chart using state energy data with circles named according to state name and sized according to state energy production.**
...
###### Example 3.2: Applying the x scale to return the circles' center x coordinates in _main.js_
            ...
            //Example 2.8 line 62, within the circle creation function
            .attr("cx", function(d, i){
...
###### Example 3.6: Creating the y axis generator in _main.js_
        ...
        //below the color scale generator...create y axis generator
...
###### Example 3.8: Inverting `.call(yAxis)` in _main.js_
        ...
        //Example 3.7...create axis g element and add axis
        var axis = container.append("g")
            .attr("class", "axis");
...
###### Example 3.9: Translating the axis 50 pixels right in _main.js_
        ...
        //Example 3.8...create axis g element and add axis
        var axis = container.append("g")
            .attr("class", "axis")
...
###### Example 3.11: Adjusting the y scale to make the axis fill the inner rectangle
        ...
        //Example 3.9 line 63...scale for circles center y coordinate
        var y = d3.scaleLinear()
            .range([450, 50]) //was 440, 95
            .domain([0, 180]); //was minEnergy, maxEnergy
...
###### Example 3.12: Adding a title to the chart
        ...
        //bottom of example 3.11... create chart title and position at the top of the map
...
A final useful touch is to label each circle with its corresponding state name and energ production. We can also use `<text>` elements for this purpose. We will need to join our `stateEnergy` data to the selection to correctly position each label (Example 3.14).
...
###### Example 3.16: Offsetting the second line in _main.js_
        ...
        //Example 3.15...second line of label
        var energyLine = labels.append("tspan")
            .attr("class", "energyLine")
            .attr("x", function (d, i) {
                //horizontal position to the right of each circle
...
*   adjust the maximum range value of our `x` scale (Example 3.16 line 49) to bring Wisconsin's label entirely into the frame.
